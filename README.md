...
— `src/config/legal.js` derives the operator name from it automatically. You'd
also want to add the registered address and a courts clause there.
...
| `/privacy` | ⚠️ drafted, placeholders filled — **needs legal review** |
| `/terms` | ⚠️ drafted, placeholders filled — **needs legal review** |
...
Two binary assets are committed, along with the scripts that produce them.
Neither runs during `npm run build` — regenerate only when the source changes.

| Asset | Source | Regenerate with |
|---|---|---|
| `public/og-default.png` (1200×630) | inline SVG in the script | `npm i --no-save sharp`<br>`node scripts/build-og-image.mjs` |
| `public/downloads/axiomrisk-risk-register-template.xlsx` | `templates/risk-register/*.csv` | `npm i --no-save exceljs`<br>`node scripts/build-risk-register.mjs` |

`sharp` and `exceljs` are deliberately **not** project dependencies — they are
heavy, only needed for these one-off generators, and would otherwise be
installed on every deploy.

The register CSVs remain the editable source of truth; the script only adds
presentation Excel can express and CSV cannot (column widths, frozen header,
1–4 validation on the rating inputs, conditional formatting on the computed
rating, dropdowns for Direction and Status). Edit the CSVs, then regenerate.

The download path is set in `src/config/resources.js` — update `file` and
`fileLabel` there if you rename or change format.
...
- [x] ~~Add `public/og-default.png`~~ — generated by `scripts/build-og-image.mjs`
- [x] ~~Add the risk register template~~ — generated by `scripts/build-risk-register.mjs`
- [x] ~~Fill the legal placeholders~~ — done; **still needs a lawyer to review**
